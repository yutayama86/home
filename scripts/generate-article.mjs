...
- 冒頭で読者の状況に触れる（2〜3行）
- 「## 結論：〜」で先に答えを出す
- 「## DXの前に可視化する業務」で、受付経路、受付時刻、担当者、案件状態、次回行動日、失注理由を整理する
- 「## 実装手順」で、対象フローを1本に絞り、各手順の入力・担当・完了条件・例外時の処理を書く
- 「## KPIの定義と見方」で、初回返信時間、未対応数、次回行動日設定率、追客実施率の定義と計算方法を書く。成果保証はしない
...
- 見積後フォローは3回分の候補を設計するが、送信間隔・文面・停止条件は会社ごとに合意し、人が承認できるようにする。
